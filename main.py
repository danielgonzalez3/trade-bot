...
def preprocess_data(df, max_bars_back):
    ...
    df = df.iloc[::-1]
    df = df.tail(max_bars_back + 50).reset_index(drop=True)
...
if __name__ == "__main__":
    price_df = stock_df(symbol="SBIN", from_date=date(2020, 1, 1), to_date=date(2023, 10, 1), series="EQ")
    ...
    plt.plot(time[0], price_df['CLOSE'].iloc[-len(predictions[0]):], label='Actual', color='red')
    plt.xlabel('Time')
    plt.ylabel('Price')
    plt.legend()
